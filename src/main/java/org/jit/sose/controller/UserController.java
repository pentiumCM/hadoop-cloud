...
import org.jit.sose.util.CryptographyUtil;
...
public class UserController {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/selectHeadImg_android.do", method = RequestMethod.POST)
    public JSONObject selectHeadImg(HttpServletRequest request){
        JSONObject headJson = new JSONObject();
        String username = request.getParameter("username");
        List<FileDto> headList = fileService.selectByFileStatus(username,"H");
        if (headList.size() > 0){
            for (int i = 0; i < headList.size(); i++) {
                FileDto head = headList.get(0);
                headJson.put("headImg",head);
            }
        }else {
            headJson.put("headImg","头像信息为空");
        }
        return headJson;
    }
