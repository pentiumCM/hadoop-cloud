...
public class FileController {
    ...
    @ResponseBody
    public JSONObject fileUpload(@RequestParam("file") CommonsMultipartFile file[], HttpServletRequest request)
            throws Exception {
     ...
     * 上传APK文件，支持多文件上传
     * @param file CommonsMultipartFile文件类
     * @param request  appFile
     * @return
     * @throws Exception
     */
    @RequestMapping(value = "/appFileUpload_android.do", method = RequestMethod.POST)
    @ResponseBody
    public JSONObject appFileUpload(@RequestParam("appFile") CommonsMultipartFile file[], HttpServletRequest request)
            throws Exception {
        JSONObject fileJson = new JSONObject();
        ...
        //判断APP的文件和配置文件里面的名字是否相同
        for (int i = 0; i < file.length ; i++) {
            String fileName = file[i].getOriginalFilename();
            if (!fileName.equals(appName)){
                fileJson.put("upFileResult","文件上传失败,上传的APK名称需符合要求");
                return fileJson;
            }
        }
        //设置apk文件上传的路径
        String filePath = properties.getProperty("apkfilepath");
        for (int i = 0; i < file.length; i++) {
            String fileName = file[i].getOriginalFilename();
